<div x-data="{ searchBox: {{ $searchBox ? 'true' : 'false' }} }" class="relative w-full" @click.away="searchBox = false">
    <div class="flex items-center max-w-md rounded-lg">
        ...
        <input type="text" wire:model.live="search" placeholder="{{ __('Search for products') }}" autocomplete=""
            ...
            <ul>
                @forelse ($results as $blog)
                    <li class="flex items-center px-4 py-3 border-b border-gray-100">
                        <a href="{{ route('front.blog', $blog->slug) }}" class="flex">
                            <img src="{{ asset('images/blogs/' . $blog->image) }}" alt="" loading="lazy"
