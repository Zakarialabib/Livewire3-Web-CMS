        ...
        <x-table>
            ...
            <x-table.tbody>
                @forelse($subscribers as $id=>$subscriber)
                    <x-table.tr wire:loading.class.delay="opacity-50" wire:key="row-{{ $subscriber->id }}">
                        <x-table.td>
                            {{ $id }}
                        </x-table.td>
                        <x-table.td>
                        ...
                        <x-table.td>
                            <livewire:utils.toggle-button :model="$subscriber" field="status"
                                key="{{ $subscriber->id }}" />
                        </x-table.td>
                        <x-table.td>
                            <x-button danger wire:loading.attr="disabled" type="button"
                                wire:click="deleteModal({{ $subscriber->id }})">
