<div>
    ...
    <div class="flex px-4 gap-4 mt-4">
        <div class="md:w-1/4 sm:w-full pb-4 px-2 border border-gray-200">
            <p class="text-lg font-semibold text-center mt-4">Upload Image</p>

            <div class="w-full grid items-center justify-center px-2">
                <x-media-upload title="{{ __('Upload Image to Gallery') }}" name="image" wire:model="image"
                    :file="$image" path="images/gallery/" single types="PNG / JPEG / WEBP" fileTypes="image/*" />
                <x-button type="button" primary wire:click="uploadImage">Upload</x-button>
            </div>
        </div>

        <div class="w-3/4 sm:w-full px-4 bg-gray-100 py-4">
            <div class="grid grid-cols-3 gap-6">
                @forelse ($this->galleries as $gallery)
                    <div class="relative " wire:key="{{ $gallery->id }}">
                        {{-- show delete top right  --}}
                        <div class="absolute top-0 right-0">
                            <x-button wire:click="removeImage({{ $gallery->id }})" danger type="button">
                                <i class="fas fa-trash"></i>
                            </x-button>
                        </div>

                        <img src="{{ asset('images/gallery/' . $gallery->image) }}" alt="Gallery Image"
                            class="w-full h-auto rounded-xl">

                        <div class="flex gap-2 absolute bottom-0 w-full bg-white bg-opacity-75 p-2"
                            x-data="{ editMode: @entangle('showEdit') }">
                            <div x-show="!editMode" class="flex gap-2 w-full justify-between">
                                <x-button wire:click="editImage('{{ $gallery->id }}')" primary type="button">
                                    <i class="fas fa-edit"></i>
                                </x-button>
                                <livewire:utils.toggle-button :model="$gallery" field="status"
                                    key="{{ $gallery->id }}" lazy />
                            </div>
                            <div x-show="editMode" class="flex gap-2 w-full" x-cloak>
                                <select class="w-full mb-2" name="tag" id="tag" wire:model="tag">
                                    @foreach ($this->services as $service)
                                        <option value="{{ $service->name }}"
                                            @if ($gallery->tag == $service->name) selected @endif>
                                            {{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
                                <x-button wire:click="updateImage" primary type="button">
                ...
                @empty
                    <div class="col-span-full text-center">
                        <p class="text-lg font-semibold">No Image Found</p>
                    </div>
                @endforelse
            </div>
        </div>
