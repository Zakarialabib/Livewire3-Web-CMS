                    ...
                    <ul id="language-dropdown"
                        ...
                        @foreach ($languages as $language)
                            @if (\Illuminate\Support\Facades\App::getLocale() !== $language->code)
                                <li class="flex">
                                    <a class="text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent hover:bg-gray-100 cursor-pointer text-gray-700"
        ...
        <ul class="flex-col md:flex-row list-none items-center md:flex">
            ...
            style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(617px, 58px);"
                ...
                <a href="{{ route('admin.profile') }}"
                    class="text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent hover:bg-gray-100 cursor-pointer text-gray-700">
                    {{ __('Profil') }}
                </a>
                ...
                @forelse(auth()->user()->alerts()->latest()->take(10)->get() as $alert)
                    <a
                        class="text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent hover:bg-gray-100 cursor-pointer text-gray-700 {{ $alert->pivot->seen_at ? 'text-gray-400' : 'text-gray-700' }}">
